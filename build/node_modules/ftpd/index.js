...
var EventEmitter, FtpClient, FtpDataPool, FtpServer, Promise, debug, http, net,
...
debug = require('debug')('yellowleaf:ftp');

FtpServer = (function(_super) {
  __extends(FtpServer, _super);

  function FtpServer(connectionListener) {
    this.ip = new Promise(function(yell, cry) {
      return http.get('https://icanhazip.com/', function(res) {
        var body;
        body = '';
        return res.on('readable', function() {
          var _ref;
          return body += (_ref = this.read()) != null ? _ref.toString() : void 0;
        }).on('end', function() {
          debug('Found IP', body);
          return yell(body.trim());
        });
      }).on('error', cry);
    });
    this.server = net.createServer((function(_this) {
      return function(socket) {
        var client;
        client = new FtpClient(socket, _this.ip);
        return _this.emit('connection', client);
      };
    })(this));
    this.server.on('error', function(err) {
      return debug('Server error:', err.message);
    });
...
FtpClient = (function(_super) {
  ...
  function FtpClient(connection, IP) {
    var host, _ref;
    host = (_ref = connection.localAddress.match(/(\d{1,3}\.){3}\d{1,3}/)) != null ? _ref[0] : void 0;
    if (!host === '127.0.0.1') {
      this.host = IP;
    } else {
      this.host = Promise.resolve('127.0.0.1');
    }
    this._connection = connection;
    this.dataServer = new FtpDataPool(this);
    this._connection.on('error', function(err) {
      return debug('Client error:', err.message);
    });

...
var EventEmitter, FtpClient, FtpDataPool, FtpServer, Promise, SuperClass, debug, fs, ftpserver, net, path,
  __slice = [].slice,
...
FtpServer = (function(_super) {
  ...
  function FtpServer(connectionListener) {
    ...
    SuperClass.inherit(this, ['address', 'listen', 'close'], this.server);
  }
...
FtpClient = (function(_super) {
  ...
  function FtpClient(connection) {
    this.host = connection.localAddress;
    ...
    this._connection.on('readable', (function(_this) {
      return function() {
        var args, cmd, data, options, _ref;
        data = _this.read();
        if (!data) {
          return;
        }
        _ref = data.split(' '), cmd = _ref[0], args = 2 <= _ref.length ? __slice.call(_ref, 1) : [];
        cmd = cmd.toLowerCase();
        options = [];

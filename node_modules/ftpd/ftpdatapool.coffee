...
class FtpDataPool extends EventEmitter
    ...
    getConnection: (cb) ->
        defered = Q['defer']()
        promise = defered.promise.then (connection) =>
            console.log 'introducting'
            @introduce()
            console.log 'Done introducing'
        ...
        process.nextTick () =>
          if @connection?
            ...
            defered.reject new Error "No connection openned, nowhere!"
          else
            defered.resolve @promiseOnce 'connection'
        promise
